...
def output_events(event, parsed, service,  attendees):
...
def parse_services(summary):
    """
    Given a calendar summary like 'ANABELA MENDES ROSTO,AX,V'
    Returns: client_name, list of services, total price, total duration
    """
    ...
    # Order matters! Check longer aliases first to avoid 'S' matching inside words
    ...
    for part in parts:
        ...
        for alias in sorted_aliases:
            if part == alias:  # exact match only, avoids 'S' matching 'COSTAS'
                service_full_name = SERVICE_ALIASES[alias]
                if service_full_name not in matched_keywords:  # no duplicates
                    matched_keywords.add(service_full_name)
                    
                    # Get price + duration from all_services
    ...
    client_name = ' '.join(name_parts).title()  # e.g. "Anabela Mendes"
...
for chunk in chunks:
    ...
    for event in events:
        ...
        if 'MULHER - ' in str(parsed['services']).upper():
            service = parsed['services'].split('Mulher - ')[-1].strip()
            print("service:",parsed['services'])
            dict_event = output_events(event, parsed, service,  attendees)
        else:
