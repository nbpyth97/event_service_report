...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    try {
      ...
      res.status(200).send(renderPage({
        ...
        message: 'Tem a certeza que quer confirmar esta marcação?',
        details: bookingDetails(booking, config.timezone),
        showButton: true,
        buttonLabel: 'Confirmar marcação',
        actionUrl: `/api/confirm?id=${encodeURIComponent(id)}&token=${encodeURIComponent(token)}`,
      }));
    } catch (err) {
      console.error(err);
  ...
  try {
    const result = await resolveBooking(id, 'confirmed');
    ...
    res.status(200).send(renderPage({
      ...
      message: 'A marcação foi confirmada com sucesso. A cliente vai ser avisada.',
